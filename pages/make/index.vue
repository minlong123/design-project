	...
	<view class="make" :class="{'scollmake':issshow}">
		...
		<view class="img-panel" @click="cancelselec" v-if="isaction"
			...
			<template v-if="isphotowall">
				...
				<view 
			class="photowall-move"
			@touchstart="moveTouchstart($event)" @touchmove="moveTouchmove" @touchend="moveTouchend"
			>
...
export default {
	...
	methods: {
		...
		moveTouchend(){
			...
			// 如果只是点击了一下，啥都不干
			if(this.movestartx == this.moveendx && this.movestarty == this.moveendy){
			...
			if (this.caisstart) {
				...
				if(this.paneltype == 1){
					if(clientX - this.castartx > 0){
						// 右边缩小
						this.photowall[this.actionbox].scale = this.photowall[this.actionbox].scale-0.08;
					}
					if(clientX - this.castartx < 0){
						// 左边放大
						this.photowall[this.actionbox].scale = this.photowall[this.actionbox].scale + 0.08;
